...
router.post(
	...
	expressAsyncHandler(async (req, res) => {
		if (!req.body.content)
			return res.status(400).json({ message: "لطفا متن نظر را وارد کنید" });
		let STATUS = 2;
...
router.put(
	...
	isAdmin,
	expressAsyncHandler(async (req, res) => {
		const comment = await PostComment.findById(req.params.commentId).populate(
			"writer",
		);
		...
		if (req.body.status === 2) {
			...
			if (comment.responseTo) {
				const reciver = await Comment.findById(comment.responseTo).populate(
					"writer",
				);
		...
		if (req.body.status === 1)
			MESSAGE = "نظر به حالت در انتظار تایید تغییر یافت";
		res.status(200).json({
			message: MESSAGE,
			comments: comment,
		});
